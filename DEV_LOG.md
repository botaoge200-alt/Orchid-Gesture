...
- **功能进度**:
  - [x] **Decal 定位修复**: 
    - 解决了 Decal 投射位置偏移问题（原在两脚间）。
    - 实现了按人体包围盒计算胸口高度。
    - 调整 Decal 默认位置：从下巴下移 0.5，并放大 10 倍。
  - [x] **交互优化**: 
    - 实现了点击胸口放置贴花（Raycaster 检测）。
    - 修复了贴花与模型的 Z-fighting/遮挡问题（沿法线微调位置）。
  - [x] **预览验证**: 本地预览正常启动。

### ⚠️ 问题记录 (Issues)
- **Git 同步**: 尝试 `git pull` 失败 (Connection refused)，需检查网络或代理设置。

### 🚧 待办事项 (Next Steps)
1.  **解决网络问题**: 重新拉取远程仓库代码并合并。
2.  **拖拽缩放**: 实现贴花的拖拽和缩放功能。
3.  **纹理恢复**: 将测试用的洋红材质切回用户上传的图片纹理。
